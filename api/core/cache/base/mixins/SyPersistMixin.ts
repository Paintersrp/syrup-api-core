...
export class SyPersistMixin<T> {
  private cacheMap: Map<string, CacheContents<T>>;
  private database: Sequelize;
  private logger: SyLogger;

  constructor(cacheMap: Map<string, CacheContents<T>>, database: Sequelize, logger: SyLogger) {
  ...
  public async saveCacheToDatabase(): Promise<void> {
    try {
      await this.executeDatabaseTransaction(async (transaction) => {
        const cacheDataObject: { [key: string]: CacheContents<T> } = {};
