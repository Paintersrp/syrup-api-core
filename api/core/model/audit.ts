...
> extends Model<TAttributes, TCreationAttributes> {
  ...
  @Field({ type: DataTypes.STRING(64), verbose: 'Model', allowNull: false })
  declare model: CreationOptional<string>;

  @Field({ type: DataTypes.JSONB, verbose: 'Before Data' })
  declare beforeData: CreationOptional<JSON>;

  @Field({ type: DataTypes.JSONB, verbose: 'After Data' })
  declare afterData: CreationOptional<JSON>;

  @Field({ type: DataTypes.STRING(500), verbose: 'User ID' })
  declare userId: CreationOptional<string>;

  @Field({ type: DataTypes.STRING(500), verbose: 'Username' })
