...
export class DatabaseOpsService {
  ...
  /**
   * Upserts multiple records into the specified model.
   *
   * @param {string} model - The name of the model into which the records should be upserted.
   * @param {Array<Optional<any, string>>} records - An array of records to be upserted.
   *
   * @returns {Promise<any>} A Promise that resolves when all upsert operations have been executed successfully, or rejects if any operation fails.
   */
  public async bulkUpsert(model: string, records: Array<Optional<any, string>>): Promise<any> {
    const operations = records.map((record) => {
      return async () => {
        await this.upsert(model, record);
      };
    });
    return await this.performBulkOperations(operations);
  }
}
